...
class RandomTransformSpace(object):
    """ Apply a random 3x4 linear transform to the world coordinate system.
        This affects pose as well as TSDF. """
    ...
    def __init__(self, voxel_dim, voxel_size, random_rotation=True, random_translation=True,
        ...
        """
        Args:
            voxel_dim: tuple of 3 ints (nx,ny,nz) specifying the size of the output volume
    ...
    def transform(self, data, transform=None, old_origin=None, align_corners=False):
        ...
        if 'tsdf_list_full' in data.keys():
            ...
            for l, tsdf_s in enumerate(data['tsdf_list_full']):
                ...
                tsdf_vol = TSDFVolumeTorch(vol_dim_s, vol_origin_partial, voxel_size=self.voxel_size * 2 ** l, margin=3)
                for i in range(data['imgs'].shape[0]):
                ...
                occ_vol = torch.zeros_like(tsdf_vol, dtype=torch.bool)
                ...
                # bilinear interpolation near surface, no interpolation along -1,1 boundry
                tsdf_vol = F.grid_sample(tsdf_s.view([1, 1] + old_voxel_dim), coords_world_s, mode='nearest',
                                         align_corners=align_corners).squeeze()
                tsdf_vol_bilin = F.grid_sample(tsdf_s.view([1, 1] + old_voxel_dim), coords_world_s, mode='bilinear',
                                               align_corners=align_corners).squeeze()
                mask = tsdf_vol.abs() < 1
