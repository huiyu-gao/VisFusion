...
def save_scalars(logger, mode, scalar_dict, global_step, count=1):
...
def coordinates(voxel_dim, device='cuda:0'):
...
def sparse_to_dense_torch(locs, values, dim, default_val, device, dtype=torch.float):
    dense = torch.full([dim[0], dim[1], dim[2]], float(default_val), device=device, dtype=dtype)
    if locs.shape[0] > 0:
...
class SaveScene(object):
    ...
    def tsdf2mesh(voxel_size, origin, tsdf_vol, layer, single_layer_mesh):
        ...
        verts = verts * voxel_size * 2 ** (2 - layer) + origin  # voxel grid coordinates to world coordinates
    ...
    def vis_incremental(self, epoch_idx, batch_idx, imgs, outputs):
        ...
        if (tsdf_volume == 1).all():
            ...
            mesh = self.tsdf2mesh(self.cfg.MODEL.VOXEL_SIZE, origin, tsdf_volume,
                                  self.cfg.MODEL.PASS_LAYERS, self.cfg.MODEL.SINGLE_LAYER_MESH)
    ...
    def save_incremental(self, epoch_idx, batch_idx, sample, outputs, save_video=True):
        save_path = os.path.join(f'incremental_{self.log_dir}_{epoch_idx}', self.scene_name, 'incremental_mesh')
    ...
    def save_scene_eval(self, epoch, outputs, batch_idx=0):
        ...
        if (tsdf_volume == 1).all():
            ...
            mesh = self.tsdf2mesh(self.cfg.MODEL.VOXEL_SIZE, origin, tsdf_volume,
                                  self.cfg.MODEL.PASS_LAYERS, self.cfg.MODEL.SINGLE_LAYER_MESH)
            # save TSDF volume for evaluation
            data = {'origin': origin,
