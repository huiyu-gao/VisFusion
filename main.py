...
if cfg.MODE == "train":
    train_dataset = Dataset(cfg.TRAIN.PATH, "train", transforms, cfg.TRAIN.N_VIEWS, len(cfg.MODEL.THRESHOLDS) - 1)
    if cfg.DISTRIBUTED:
...
if cfg.TRAIN.FINETUNE_LAYER is None:
    # train the whole model
    params = model.parameters()
else:
    # fine tune a specific layer in the coarse_to_fine_recon network
    layer = cfg.TRAIN.FINETUNE_LAYER
    assert isinstance(layer, int), "FINETUNE_LAYER should be an integer"
    params = list(model.module.coarse_to_fine_recon.visible_fusion.view_select[layer].parameters()) + \
             list(model.module.coarse_to_fine_recon.sp_convs[layer].parameters()) + \
             list(model.module.coarse_to_fine_recon.local_occ_preds[layer].parameters()) + \
             list(model.module.coarse_to_fine_recon.local_tsdf_preds[layer].parameters()) + \
             list(model.module.coarse_to_fine_recon.gru_fusion.fusion_nets[layer].parameters()) + \
             list(model.module.coarse_to_fine_recon.gru_fusion.tsdf_mlp[layer].parameters()) + \
             list(model.module.coarse_to_fine_recon.global_occ_preds[layer].parameters()) + \
             list(model.module.coarse_to_fine_recon.global_tsdf_preds[layer].parameters())
            ...
            try:
                ...
                if is_main_process():
                    logger.info('Epoch {}/{}, Iter {}/{}, train loss = {:.3f}, time = {:.3f}'.format(epoch_idx,
                                 cfg.TRAIN.EPOCHS, batch_idx, len(TrainImgLoader), loss, time.time() - start_time))
...
def train_sample(sample):
    if cfg.TRAIN.FINETUNE_LAYER is None:
        ...
        model.eval()
        model.module.coarse_to_fine_recon.visible_fusion.view_select[layer].train()
        model.module.coarse_to_fine_recon.sp_convs[layer].train()
        model.module.coarse_to_fine_recon.local_occ_preds[layer].train()
        model.module.coarse_to_fine_recon.local_tsdf_preds[layer].train()
        model.module.coarse_to_fine_recon.gru_fusion.fusion_nets[layer].train()
        model.module.coarse_to_fine_recon.gru_fusion.tsdf_mlp[layer].train()
        model.module.coarse_to_fine_recon.global_occ_preds[layer].train()
        model.module.coarse_to_fine_recon.global_tsdf_preds[layer].train()
    ...
    optimizer.zero_grad()
    outputs, loss_dict = model(sample, finetune_layer=cfg.TRAIN.FINETUNE_LAYER)
